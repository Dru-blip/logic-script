/**
 * @file A TypeScript implementation of the `Optional` type, inspired by Java's `Optional<T>`.
 * This module provides a functional approach to handling nullable values by encapsulating them
 * within an `Optional` container, allowing safe transformations and operations without null checks.
 *
 * @author [bromne] {@link https://github.com/bromne}
 * @Modified by [Dru-blip] {@link https://github.com/Dru-blip}
 *
 * @see {@link https://github.com/bromne/typescript-optional} for original implementation
 */
...
export abstract class Optional<T> {
  ...
  static of<T>(value: T): Optional<T> {
    if (value !== null && value !== undefined) return new Some<T>(value);
  ...
  static ofNullable<T>(nullable: T | null | undefined): Optional<T> {
    if (nullable !== null && nullable !== undefined)
      return new Some<T>(nullable);
    else return new Empty<T>();
  }
...
class Some<T> extends Optional<T> {
  ...
  matches<U>(cases: Cases<T, U>): U {
    return cases.some(this.payload);
  }
